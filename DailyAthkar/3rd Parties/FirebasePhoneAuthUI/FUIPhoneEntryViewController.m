...
- (void)viewWillAppear:(BOOL)animated {
  ...
  if (self.navigationController.viewControllers.firstObject == self) {
    if (self.authUI.providers.count != 1){
      ...
      UIBarButtonItem *cancelBarButton = [[UIBarButtonItem alloc] initWithTitle:backLabel style:UIBarButtonItemStylePlain target:self action:@selector(cancelAuthorization)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (indexPath.row == FUIPhoneEntryRowCountrySelector) {
    ...
    _phoneNumberField.keyboardType = UIKeyboardTypeASCIICapableNumberPad;
